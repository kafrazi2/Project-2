...
rawDataNew <- rawData %>% mutate(datachannel =   if_else(data_channel_is_bus == 1, "Business",
...
## AUTOMATION HERE?? REMOVE THIS COMMENT!! ##
library(dplyr)
subsetData <- rawDataNew %>% filter( datachannel == "Entertainment")
...
df<- data.frame(t(fitLM1$results), t(fitLM2$results), t(fitLM3$results))
df
