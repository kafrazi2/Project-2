...
First linear fit with day of the week as predictor and shares as response.
```{r}
fit1 <- train(shares ~ day_of_the_week, data = trainDataNew, method = "lm", trControl = trainControl(method = "cv", number = 10))
fit1
```

Second linear fit with day of the week, number of images, and number of videos as predictors and shares as response.
```{r}
fit2 <- train(shares ~ day_of_the_week + num_imgs + num_videos, data = trainDataNew, method = "lm", trControl = trainControl(method = "cv", number = 10))
fit2
```

Third linear fit with the interaction of day of the week and number of images and shares as response.
```{r}
fit3 <- train(shares ~ day_of_the_week*num_imgs, data = trainDataNew, method = "lm", trControl = trainControl(method = "cv", number = 10))
fit3
```
...
### How to Pick Values for Tuning Parameters?

n.trees <- c(25, 50,100, 150, 250)
...
boostFit <- train(shares ~ ., data = trainData, method = "gbm", trControl = trainControl(method = "cv", number = 5), tuneGrid = df)

## Model Comparison and Selection 

```{r comparison}

## Add RF and Boosted Models ##

df<- data.frame(t(fit1$results), t(fit2$results), t(fit3$results), t(fitLM$results))
...
## Update with Best Model Selection ##

## Fit Best Model to Test Data
pred <- predict(fiLM, newdata = testData)
confusionMatrix(testData$shares, pred)
